...
async def check_if_editing(message: Message, state: FSMContext, attribute_name: str, bot: Bot = None):
    ...
    if state_data.get("editing_attribute") == attribute_name:
        ...
        # Проверяем, редактирует ли админ
        if state_data.get("is_admin_edit"):
            # Используем обработчик для админского редактирования
            from app.handlers.admin import show_admin_supplier_confirmation
            await show_admin_supplier_confirmation(message, state, bot)
        else:
            # Стандартный обработчик для пользователя
...
async def process_main_category(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        # Проверяем, находимся ли мы в режиме редактирования
        # Для админа не завершаем процесс здесь, так как нужно выбрать ещё и подкатегорию
        is_admin_edit = state_data.get("is_admin_edit", False)
        if not is_admin_edit and await check_if_editing(message, state, "main_category", bot):
            return
...
async def process_subcategory(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        # Если это редактирование администратором, проверяем редактирование "главной категории"
        # так как основная категория и подкатегория - это один пункт меню в редактировании
        is_admin_edit = state_data.get("is_admin_edit", False)
        if is_admin_edit and state_data.get("editing_attribute") == "main_category":
            # Обновляем оба поля в состоянии для редактирования
            from app.handlers.admin import show_admin_supplier_confirmation
            await message.answer(f"Категория и подкатегория обновлены на: {selected_category} > {selected_subcategory['name']}")
            await state.update_data(editing_attribute=None)
            await show_admin_supplier_confirmation(message, state, bot)
            return
        
        # Проверяем, находимся ли мы в обычном режиме редактирования
